...
A continuación para detectar los bordes con Sobel, se calculan los gradientes horizontales (`sobelx`) y verticales (`sobely`) usando el operador Sobel, se obtiene la magnitud del gradiente combinando ambos gradientes y se convierte a 8 bits (`sobel8u`).  
